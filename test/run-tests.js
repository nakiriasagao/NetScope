 ...
 * 用法：
 *   cd D:\DSH\NetScope
 *   node test/run-tests.js            # 单元测试 + 接口冒烟（服务在跑时才执行）
 ...
 *   3. 输出总体结论，并以合适的 exit code 退出（0=全部通过/按需跳过，1=有失败）。
 *
 * 环境变量：
 *   NS_BASE=http://127.0.0.1:8787   冒烟测试的服务地址（与 smoke-api.js 保持一致）
...
(async () => {
  ...
  /* ---------- 1. 单元测试 ---------- */
  say('---------------- [1/2] 单元测试 (test/unit.test.js) ----------------');
  ...
  /* ---------- 2. 接口冒烟 ---------- */
  say('---------------- [2/2] 接口冒烟 (test/smoke-api.js) ----------------');
  ...
  if (failed.length) {
    ...
    say('注意：单元测试中的「已知缺陷」用例为 skip 状态，不计入失败；其清单见 test/unit.test.js 末尾。');
    process.exitCode = 1;
    return;
  }
